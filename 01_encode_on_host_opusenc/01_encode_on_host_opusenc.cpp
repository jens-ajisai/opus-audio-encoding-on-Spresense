...
void doEncode(const char* filenameIn, const char* filenameOut) {
  ...
  enc = ope_encoder_create_file(filenameOut, comments, 48000, 1, 0,
                                &error);
  if (!enc) {
...
int main() {
  ...
  const char *testAudioFileNameOut = "../00_preparation/test_c1_48000_s16.ogg";

  doEncode(testAudioFileNameIn, testAudioFileNameOut);
}